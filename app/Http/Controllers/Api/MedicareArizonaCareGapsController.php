...
class MedicareArizonaCareGapsController extends Controller
{
    protected $per_page = '';
    ...
    {    
        try{
            ...
            $query = MedicareArizonaCareGaps::where('deleted_at',NULL);
            if(!empty($doctor_id)){
    ...
    public function storeBulkMedicareArizonaCareGaps(Request $request)
    {
        ...
        try {
                ...
                $lastDataInsert = MedicareArizonaCareGaps::select('id','created_at')->where('source','like','%CareGap_File%')->where('gap_year', $gap_year)->orderBy('id', 'desc')->first();
                ...
                if ($months >= 3){
                    ...
                    ** Don't forget to add the new gap in this array if new gap is in system database */
                    $constantsCareGaps = Config('constants.caregaps.med-arz-001');

                    // checking the records of non excel created gaps to delete when creating the gaps from sheet
                    if ($gap_year == $currentYear) {
                        $currentYearGaps = MedicareArizonaCareGaps::where('gap_year', $currentYear)->where('source', '!=', 'CareGap_File')->delete();
                    }
                    ...
                    if (!empty($bulktoStoreCareGaps)) {
                        $res = MedicareArizonaCareGaps::insert($bulktoStoreCareGaps);
                        ...
                        if ($res) {
                            ...
                            $modelInstance = new MedicareArizonaCareGaps();
                            ...
                            $insertedRows = MedicareArizonaCareGaps::whereIn('id', $insertedIds)->get();
                        }
            ...
            //     $lastDataInsert = MedicareArizonaCareGaps::select('id','created_at')->where('source','like','%MedicareArizonaCareGap_File%')->orderBy('id', 'desc')->first();
            ...
            //     $valueOfCareGaps = [];
            //     $bulktoStoreMedicareArizonaCareGaps = [];
            //     foreach ($data as $key => $value) {
            //         $breastCancerGap = $colorectalCancerGap = $bpGap = $EyeExamGap = $HBA1CGap = $statinGap = "N/A";
            ...
            //             $valueOfCareGaps["source"]          = 'MedicareArizonaCareGap_File';
            //             $valueOfCareGaps["created_user"]    = Auth::id();
            ...
            //             $bulktoStoreMedicareArizonaCareGaps[] = (array)$valueOfCareGaps;
            //             // echo "<pre>";
            //             // print($bulktoStoreMedicareArizonaCareGaps);
            //         } else {
            //             $missedPatients[] = $whereClause;
            //         }
            //     }

            //    if (!empty($bulktoStoreMedicareArizonaCareGaps)) {
            //         $res = MedicareArizonaCareGaps::insert($bulktoStoreMedicareArizonaCareGaps);
            //         $response = [
            //             'success' => true,
            //             'message'=> 'MedicareArizonaCareGaps Added Successfully And '.$patientConcrollerResponse,
            //             // 'duplicate_patients_ids' => $duplicate_patients_ids
            //         ];
            //         //$response = array('success'=>true,'message'=> 'MedicareArizonaCareGaps Added Successfully And '.$patientConcrollerResponse);
            ...
            //         ];
            //         //$response = array('success'=>false,'message'=>'Duplicate MedicareArizonaCareGaps found And '.$patientConcrollerResponse);
            //     }
            } catch (\Exception $e) {
                $response = array('success'=>false,'message'=>$e->getMessage(), 'line'=>$e->getLine());
            }
    ...
    public function update(Request $request, $id)
    {
        ...
        $folder_name = '/MedicareArizonaCareGaps/';//$request->folder_name ?? "";
        ...
        $note = MedicareArizonaCareGaps::find($id);
        ...
        if ($id != 0) {
            ...
            $gapInsert = MedicareArizonaCareGaps::create($input121);
        ...
        if ($id==0) {
            $note = MedicareArizonaCareGaps::find($gapInsert->id);
        }
        ...
        try {
            ...
            if($request->col_name =='bp_control_gap') {
