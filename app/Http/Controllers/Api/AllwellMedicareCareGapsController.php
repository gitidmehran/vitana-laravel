...
class AllwellMedicareCareGapsController extends Controller
{
    protected $per_page = '';
    ...
    {    
        try{
            ...
            $query = AllwellMedicareCareGaps::where('deleted_at',NULL);
            if(!empty($doctor_id)){
    ...
    public function storeBulkAllwellMedicareCareGaps(Request $request)
    {   
        ...
        try {
                ...
                $lastDataInsert = AllwellMedicareCareGaps::select('id','created_at')->where('source','like','%CareGap_File%')->where('gap_year', $gap_year)->orderBy('id', 'desc')->first();
                ...
                $patientsResult = (new PatientsController)->storeBulkPatients($request, 1, $gap_year)->getOriginalContent();
                    ...
                    ** Don't forget to add the new gap in this array if new gap is in system database */
                    $constantsCareGaps = Config('constants.caregaps.allwell-001');

                    // checking the records of non excel created gaps to delete when creating the gaps from sheet
                    if ($gap_year == $currentYear) {
                        $currentYearGaps = AllwellMedicareCareGaps::where('gap_year', $currentYear)->where('source', '!=', 'CareGap_File')->delete();
                    }
                    ...
                    if (!empty($bulktoStoreCareGaps)) {
                        ...
                        foreach ($bulktoStoreCareGaps as $key => $value) {
                            ...
                            $res = AllwellMedicareCareGaps::updateOrCreate($condition, $value);
                        }
                        ...
                        if ($res) {
                            ...
                            $modelInstance = new AllwellMedicareCareGaps();
                            ...
                            $insertedRows = AllwellMedicareCareGaps::whereIn('id', $insertedIds)->get();
                        }
                        ...
                        // return response()->json($scheduledAwv);

                        if (!empty($scheduledAwv)) {
                            CareGapsDetails::insert($scheduledAwv);
                        }

                        $response = array('success'=>true, 'scheduled'=> $scheduledAwv, 'error'=> 'CareGaps Added Successfully And '.$patientConcrollerResponse);
                    } else {
                ...
                // } else {
                //     $response = array('success'=>true,'error'=> 'Duplicate data found');
                // }

//                 $lastDataInsert = AllwellMedicareCareGaps::select('id','created_at')->where('source','like','%AllwellMedicareCareGap_File%')->orderBy('id', 'desc')->first();
...
//                     $response = [
//                         'success' => true,
//                         'error'=>'You have already uploaded Care gaps within last 3 Months',
//                     ];
//                     return response()->json($response);
//                 }
...
//                 $valueOfCareGaps = [];
//                 $bulktoStoreAllwellMedicareCareGaps = [];
//                 foreach ($data as $key => $value) {
//                     $breastCancerGap = $colorectalCancerGap = $bpGap = $EyeExamGap = $HBA1CGap = $statinGap = "N/A";
...
// //4
//                         if (@$value['med_adherence_ras_gap'] == 'Compliant' && @$value['med_adherence_ras_gap_current'] != 'Compliant') {
//                             $value['med_adherence_ras_gap_current'] = 'Compliant';
//                         }
//                         $valueOfCareGaps["med_adherence_ras_gap"] = @$value['med_adherence_ras_gap_current'] ?? 'N/A';
//                         $valueOfCareGaps["med_adherence_ras_gap_insurance"] = @$value['med_adherence_ras_gap'] ?? 'N/A';
//     //7
                        
//                         if (@$value['hba1c_gap'] == 'Compliant' && @$value['hba1c_gap_current'] != 'Compliant') {
//                             $valueOfCareGaps["hba1c_gap"] = $valueOfCareGaps["hba1c_gap_insurance"] = 'Compliant';
//                             $value['hba1c_gap_current'] = 'Compliant';
//                         }
//                         else if (@$value['hba1c_gap'] != 'Compliant'){
//                             $valueOfCareGaps["hba1c_gap"] = @$value['hba1c_gap_current'] ?? $HBA1CGap;
//                             $valueOfCareGaps["hba1c_gap_insurance"] = @$value['hba1c_gap'] ?? $HBA1CGap;
//                         }
//                         else if (@$value['hba1c_gap'] == 'Compliant'){
//                             $valueOfCareGaps["hba1c_gap"] = @$value['hba1c_gap_current'] ?? $HBA1CGap;
//                             $valueOfCareGaps["hba1c_gap_insurance"] = @$value['hba1c_gap'] ?? $HBA1CGap;
//                         }
//                         // $valueOfCareGaps["hba1c_gap"] = @$value['hba1c_gap_current'] ?? 'N/A';
//                         // $valueOfCareGaps["hba1c_gap_insurance"] = @$value['hba1c_gap'] ?? 'N/A';
// //5
//                         if (@$value['med_adherence_statins_gap'] == 'Compliant' && @$value['med_adherence_statins_gap_current'] != 'Compliant') {
//                             $value['med_adherence_statins_gap_current'] = 'Compliant';
//                         }
//                         $valueOfCareGaps["med_adherence_statins_gap"] = @$value['med_adherence_statins_gap_current'] ?? 'N/A';
//                         $valueOfCareGaps["med_adherence_statins_gap_insurance"] = @$value['med_adherence_statins_gap'] ?? 'N/A';
//     //8
...
//     //9
//                         if (@$value['pain_screening_gap'] == 'Compliant' && @$value['pain_screening_gap_current'] != 'Compliant') {
//                             $value['pain_screening_gap_current'] = 'Compliant';
//                         }
//                         $valueOfCareGaps["pain_screening_gap"] = @$value['pain_screening_gap_current'] ?? 'N/A';
//                         $valueOfCareGaps["pain_screening_gap_insurance"] = @$value['pain_screening_gap'] ?? 'N/A';
//     //10
//                         if (@$value['adults_medic_gap'] == 'Compliant' && @$value['adults_medic_gap_current'] != 'Compliant') {
//                             $value['adults_medic_gap_current'] = 'Compliant';
//                         }
//                         $valueOfCareGaps["adults_medic_gap"] = @$value['adults_medic_gap_current'] ?? 'N/A';
//                         $valueOfCareGaps["adults_medic_gap_insurance"] = @$value['adults_medic_gap'] ?? 'N/A';
//     //11                    
//                         if (@$value['m_high_risk_cc_gap'] == 'Compliant' && @$value['m_high_risk_cc_gap_current'] != 'Compliant') {
//                             $value['m_high_risk_cc_gap_current'] = 'Compliant';
//                         }
//                         $valueOfCareGaps["m_high_risk_cc_gap"] = @$value['m_high_risk_cc_gap_current'] ?? 'N/A';
//                         $valueOfCareGaps["m_high_risk_cc_gap_insurance"] = @$value['m_high_risk_cc_gap'] ?? 'N/A';
//     //12
//                         if (@$value['trc_eng_after_disc_gap'] == 'Compliant' && @$value['trc_eng_after_disc_gap_current'] != 'Compliant') {
//                             $value['trc_eng_after_disc_gap_current'] = 'Compliant';
//                         }
//                         $valueOfCareGaps["trc_eng_after_disc_gap"] = @$value['trc_eng_after_disc_gap_current'] ?? 'N/A';
//                         $valueOfCareGaps["trc_eng_after_disc_gap_insurance"] = @$value['trc_eng_after_disc_gap'] ?? 'N/A';
//     //13
//                         if (@$value['trc_mr_post_disc_gap'] == 'Compliant' && @$value['trc_mr_post_disc_gap_current'] != 'Compliant') {
//                             $value['trc_mr_post_disc_gap_current'] = 'Compliant';
//                         }
//                         $valueOfCareGaps["trc_mr_post_disc_gap"] = @$value['trc_mr_post_disc_gap_current'] ?? 'N/A';
//                         $valueOfCareGaps["trc_mr_post_disc_gap_insurance"] = @$value['trc_mr_post_disc_gap'] ?? 'N/A';
//     //14
//                         if (@$value['med_adherence_diabetic_gap'] == 'Compliant' && @$value['med_adherence_diabetic_gap_current'] != 'Compliant') {
//                             $value['med_adherence_diabetic_gap_current'] = 'Compliant';
//                         }
//                         $valueOfCareGaps["med_adherence_diabetic_gap"] = @$value['med_adherence_diabetic_gap_current'] ?? 'N/A';
//                         $valueOfCareGaps["med_adherence_diabetic_gap_insurance"] = @$value['med_adherence_diabetic_gap'] ?? 'N/A';
//     //15
//                         if (@$value['kidney_health_diabetes_gap'] == 'Compliant' && @$value['kidney_health_diabetes_gap_current'] != 'Compliant') {
//                             $value['kidney_health_diabetes_gap_current'] = 'Compliant';
//                         }
//                         $valueOfCareGaps["kidney_health_diabetes_gap"] = @$value['kidney_health_diabetes_gap_current'] ?? 'N/A';
//                         $valueOfCareGaps["kidney_health_diabetes_gap_insurance"] = @$value['kidney_health_diabetes_gap'] ?? 'N/A';
                        
//     //16
//                         if (@$value['spc_statin_therapy_cvd_gap'] == 'Compliant' && @$value['spc_statin_therapy_cvd_gap_current'] != 'Compliant') {
//                             $value['spc_statin_therapy_cvd_gap_current'] = 'Compliant';
//                         }
//                         $valueOfCareGaps["spc_statin_therapy_cvd_gap"] = @$value['spc_statin_therapy_cvd_gap_current'] ?? 'N/A';
//                         $valueOfCareGaps["spc_statin_therapy_cvd_gap_insurance"] = @$value['spc_statin_therapy_cvd_gap'] ?? 'N/A';
//     //17
...
//                         }
//                         // $existPatientData = Patients::where('member_id', $member_id)->orWhere('unique_id', $unique_id)->get()->toArray();
...
//                         $valueOfCareGaps["source"]          = 'AllwellMedicareCareGap_File';
//                         $valueOfCareGaps["created_user"]    = Auth::id();
...
//                         $bulktoStoreAllwellMedicareCareGaps[] = (array)$valueOfCareGaps;
//                     } else {
//                         $missedPatients[] = $whereClause;
//                     }
//                 }
                
//                 if (!empty($bulktoStoreAllwellMedicareCareGaps)) {
//                     $res = AllwellMedicareCareGaps::insert($bulktoStoreAllwellMedicareCareGaps);
//                     $response = [
//                         'success' => true,
//                         'error'=> 'AllwellMedicareCareGaps Added Successfully And '.$patientConcrollerResponse,
//                         'duplicate_patients_ids' => $duplicate_patients_ids
//                     ];
//                     //$response = array('success'=>true,'error'=> 'AllwellMedicareCareGaps Added Successfully And '.$patientConcrollerResponse);
                
//                 } else {
//                     $response = [
//                         'success' => true,
//                         'error'=>'No data to insurt ',
//                     ];
//                     //$response = array('success'=>false,'error'=>'Duplicate AllwellMedicareCareGaps found And '.$patientConcrollerResponse);
//                 }
            } catch (\Exception $e) {
    ...
    public function update(Request $request, $id)
    {
        ...
        $folder_name = '/AllwellMedicareCareGaps/';//$request->folder_name ?? "";
        ...
        $note = AllwellMedicareCareGaps::find($id);
        ...
        if ($id != 0) {
            ...
            $gapInsert = AllwellMedicareCareGaps::create($input121);
        ...
        if ($id==0) {
            $note = AllwellMedicareCareGaps::find($gapInsert->id);
        }
