...
class AetnaMedicareCareGapsController extends Controller
{
    protected $per_page = '';
    ...
    {    
        try{
            ...
            $query = AetnaMedicareCareGaps::where('deleted_at',NULL);
            if(!empty($doctor_id)){
    ...
    public function storeBulkAetnaMedicareCareGaps(Request $request)
    {
        ...
        try {
                ...
                $lastDataInsert = AetnaMedicareCareGaps::select('id','created_at')->where('source','like','%CareGap_File%')->where('gap_year', $gap_year)->orderBy('id', 'desc')->first();
                ...
                if ($months >= 3){
                    ...
                    ** Don't forget to add the new gap in this array if new gap is in system database */
                    $constantsCareGaps = Config('constants.caregaps.aet-001');

                    // checking the records of non excel created gaps to delete when creating the gaps from sheet
                    if ($gap_year == $currentYear) {
                        $currentYearGaps = AetnaMedicareCareGaps::where('gap_year', $currentYear)->where('source', '!=', 'CareGap_File')->delete();
                    }
                    ...
                    if (!empty($bulktoStoreCareGaps)) {
                        $res = AetnaMedicareCareGaps::insert($bulktoStoreCareGaps);
                        ...
                        if ($res) {
                            ...
                            $modelInstance = new AetnaMedicareCareGaps();
                            ...
                            $insertedRows = AetnaMedicareCareGaps::whereIn('id', $insertedIds)->get();
                        }
                ...
                // previous code
//                 $lastDataInsert = AetnaMedicareCareGaps::select('id','created_at')->where('source','like','%AetnaMedicareCareGaps_File%')->orderBy('id', 'desc')->first();
...
//                 $valueOfCareGaps = [];
//                 $bulktoStoreAetnaMedicareCareGaps = [];
//                 foreach ($data as $key => $value) { 
//                     $breastCancerGap = $colorectalCancerGap = $bpGap = $EyeExamGap = $HBA1CGap = $statinGap = "N/A";
...
//                     $existPatientID = @$existPatientData[0]['id'] ?? 0 ;
...
//                                 $valueOfCareGaps["hba1c_gap"] = $valueOfCareGaps["hba1c_poor_gap_insurance"] = 'Compliant';
...
//                         $valueOfCareGaps["source"]          = 'AetnaMedicareCareGap_File';
//                         $valueOfCareGaps["created_user"]    = Auth::id();
//                         $valueOfCareGaps["patient_id"]      = @$existPatientID;
//                         $valueOfCareGaps["created_at"]      = Carbon::now();
//                         $valueOfCareGaps["updated_at"]      = Carbon::now();
//                         $valueOfCareGaps["deleted_at"]       = NULL;
                        
//                         $bulktoStoreAetnaMedicareCareGaps[] = (array)$valueOfCareGaps;
//                     }
//                     else {
//                         //$missedPatients[] = $whereClause;
//                     }
//                 }
// //return $bulktoStoreAetnaMedicareCareGaps;
//                 if (!empty($bulktoStoreAetnaMedicareCareGaps)) {
//                     $res = AetnaMedicareCareGaps::insert($bulktoStoreAetnaMedicareCareGaps);
//                     $response = [
//                         'success' => true,
//                         'message'=> 'AetnaMedicareCareGaps Added Successfully And '.$patientConcrollerResponse,
//                         //'duplicate_patients_ids' => $duplicate_patients_ids
//                     ];
//                     //$response = array('success'=>true,'message'=> 'AetnaMedicareCareGaps Added Successfully And '.$patientConcrollerResponse);
...
//                     ];
//                     //$response = array('success'=>false,'message'=>'Duplicate AetnaMedicareCareGaps found And '.$patientConcrollerResponse);
//                 }
            } catch (\Exception $e) {
                $response = array('success'=>false,'message'=>$e->getMessage(), 'line'=>$e->getLine());
            }
    ...
    public function update(Request $request, $id)
    {
        ...
        $folder_name = '/AetnaMedicareCareGaps/';//$request->folder_name ?? "";
        ...
        $note = AetnaMedicareCareGaps::find($id);
        ...
        if ($id != 0) {
            ...
            $gapInsert = AetnaMedicareCareGaps::create($input121);
        ...
        if ($id==0) {
            $note = AetnaMedicareCareGaps::find($gapInsert->id);
        }
