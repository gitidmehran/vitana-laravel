<form method="post" action="{{url($action.'/'.$row['id'])}}" class="make_file_ajax" enctype="multipart/form-data">
 ...
 -->
            ...
            <div class="col-6 my-2">
                <div class="form-group">
                    <label for="name">Clinic Name<span style="color:red;">*</span></label>
                    
                    <select class="form-select" name="clinic_id" id="clinic_id" required="required">
                        <option value="">-- Select Clinic --</option>
                        
                        @foreach($clinicName as $key => $val)
                            <option value="{{$val->id}}" @if($val->id==$clinic_user->clinic_id) selected @endif>{{$val->name}}</option>
                        @endforeach
