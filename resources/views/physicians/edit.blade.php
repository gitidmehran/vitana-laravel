<form method="post" action="{{url($action.'/'.$row['id'])}}" class="make_file_ajax" enctype="multipart/form-data">
            ...
            <div class="col-6 my-2">
                <div class="form-group">
                    <label for="role">Designation <span style="color:red;">*</span></label>
                       @php
                       $userRole = ['Physicians'=>'5'];
    @endphp

                    <select name="role" id="role" required class="form-control" value= "{{@$row['role']}}">
 ...
 -->
            ...
            <div class="col-6 my-2">
                <div class="form-group">
                    <label for="contact_no">Contact <span style="color:red;">*</span></label>
            ...
            <div class="col-12 my-2">
                <div class="form-group">
                   ...
                   <!--  <input type="text" class="form-control" id="speciality" name="speciality" placeholder="Doctor Speciality" value="{{@$row['speciality']}}"> -->
                   <select class="form-select" name="speciality" required id="speciality" value= "{{@$row['name']}}">>
                        <option value="">Select Speciality</option>

                        @foreach($physiciansSpeciality as $key => $val)
                           
                            <option value="{{$val['name']}}" @if($val['name']==$row['speciality']) selected @endif>{{$val['name']}}</option>
                        @endforeach 
